#!/bin/bash
#SBATCH --job-name=cpump_ML_02
#SBATCH --chdir=./
#SBATCH --output=./logfiles/cpump_ML_02.%A_%3a.out  # %J=jobid.step, %N=node.
#
...
echo "Running task $SLURM_ARRAY_TASK_ID specified in logfiles/cpump_ML_02.config.pkl on $HOSTNAME at $(date) with $USE_NUM_THREADS threads"
python /home/t30/kna/ge54yin/Documents/MasterThesisFQHinTMDs/MPS_stuff/MPScalculations/cluster_jobs.py run logfiles/cpump_ML_02.config.pkl $SLURM_ARRAY_TASK_ID
# if you want to redirect output to file, you can append the following to the line above:
#     &> "cpump_ML_02.task_$SLURM_ARRAY_TASK_ID.out"
